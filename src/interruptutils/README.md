...
in the popup; it is read only for a rule whose class, title and process already matched (reading it
is a call into the popup's application), so set `titleContains` or `processName` as well. Rules are
tried in the order added and the first match wins. See [Rules](Documentation/Rules.md).
