...
```csharp
mouse.ClickAt(1180, 860, MouseButton.Left);
```
...
```csharp
mouse.DoubleClickAt(340, 512, MouseButton.Left); // opens the selected invoice
```
...
```csharp
mouse.RightClick();          // open the tray icon's context menu
Thread.Sleep(200);
mouse.MoveTo(exitX, exitY);
mouse.LeftClick();           // choose "Exit"

mouse.MiddleClick();         // close the current browser tab under the cursor
```
...
```csharp
mouse.RightClickAt(410, 260); // context menu on cell B7
mouse.LeftClickAt(150, 40);   // Paste button in the ribbon
```
...
```csharp
mouse.MoveTo(columnBorderX, headerY);
mouse.LeftDoubleClick(); // auto-fit column width
```
...
```csharp
mouse.LeftDoubleClickAt(48, 96); // "Order Entry" desktop icon
```
...
mouse.MoveTo(startX + 50, startY - 30); // detour around an obstacle control
...
mouse.ClickAndHold(MouseButton.Left, 500);
```
...
```csharp
mouse.MoveTo(rowY: 200, x: 100); // pseudo-args for illustration
mouse.ClickWithModifiers(MouseButton.Left, ModifierKeys.Control);

mouse.MoveTo(320, 100);
mouse.ClickWithModifiers(MouseButton.Left, ModifierKeys.Control);

mouse.MoveTo(560, 100);
mouse.ClickWithModifiers(MouseButton.Left, ModifierKeys.Control | ModifierKeys.Shift);
```
...
mouse.ClickAndRestore(dismissX: 960, dismissY: 40, MouseButton.Left);
```

## `ClickWithRetry(int x, int y, MouseButton button, int maxAttempts, int retryDelayMilliseconds)`

**Scenario:** An unattended bot occasionally hits a `Win32Exception` clicking
"Submit" because a Windows Update notification balloon briefly grabs the
secure desktop. Rather than failing the whole case, the automation retries
the click a few times with a short delay before giving up for real.

```csharp
mouse.ClickWithRetry(x: 640, y: 480, MouseButton.Left, maxAttempts: 3, retryDelayMilliseconds: 500);
```
...
```csharp
mouse.MoveTo(notesFieldX, notesFieldY);
mouse.TripleClick(MouseButton.Left); // selects the whole line
