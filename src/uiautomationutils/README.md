...
| `GetDescendantsSummaryJson` | `bool GetDescendantsSummaryJson(AutomationElement parent, out string json, out bool truncated, out string message, int maxDepth = 5, int maxNodes = 500)` | Same as `GetChildrenSummaryJson`, but recurses into each child's own children too, nesting them under a `children` array - for discovering a control's full layout without knowing its structure or depth ahead of time. `maxDepth` (1-20) bounds how deep it recurses; `maxNodes` (1-20000, default 500) bounds the total elements included - this, not `maxDepth`, is what protects against a *wide* subtree. `truncated` reports whether `maxNodes` cut the result short. |
...
| `GetDescendantsSummaryJsonFromWindowHandle` | `bool GetDescendantsSummaryJsonFromWindowHandle(IntPtr hWnd, out string json, out bool truncated, out string message, int maxDepth = 5, int maxNodes = 500)` | Same as `GetChildrenSummaryJsonFromWindowHandle`, but recurses the full subtree via `GetDescendantsSummaryJson`. |
...
- **`GetChildren`/`FindAllByControlType` results feed `GetElementCount`/`GetElementAt`**
  ...
  `GetDescendantsSummaryJson` goes further still, recursing the entire subtree into the
  same JSON shape with a nested `children` array per node - for discovering an
  unfamiliar control's full layout in one call. `maxDepth` bounds path length only;
  `maxNodes` bounds the total element count (what actually protects against a *wide*
  subtree, e.g. a browser-hosted control with hundreds of children per level) and sets
  `truncated` if it cuts the result short.
