...
namespace WinEventAutomation
{
    ...
    /// <see cref="WinEventCategory"/> set it belongs to. An event can map to more
    /// than one category (a "#32770" dialog is both a window-created event and a
    /// dialog event).
    /// </summary>
    internal static class WinEventCategoryMap
    {
        public static string WinEventName(uint eventType)
        {
            switch (eventType)
            {
        ...
        public static List<WinEventCategory> CategoriesForEvent(uint eventType, string className)
        {
            var cats = new List<WinEventCategory>(2);
            switch (eventType)
            {
                ...
                case WinEventInterop.EVENT_SYSTEM_MENUPOPUPEND:
                    cats.Add(WinEventCategory.Menus);
                    break;
                ...
                case WinEventInterop.EVENT_SYSTEM_MOVESIZEEND:
                    cats.Add(WinEventCategory.WindowOps);
                    break;
                case WinEventInterop.EVENT_SYSTEM_SWITCHSTART:
                    cats.Add(WinEventCategory.Session);
                    break;
            }
    ...
    /// <see cref="WinEventData"/>. One hook covers the whole SYSTEM+OBJECT range;
    ...
    internal sealed class WinEventEngine : IDisposable
    {
        ...
        private readonly Action<WinEventData, List<WinEventCategory>> _consumer;
        private readonly WinEventInterop.WinEventProcDelegate _delegateRef; // kept alive for the hook's lifetime
        private readonly ConcurrentQueue<WinEventData> _ring = new ConcurrentQueue<WinEventData>();
        ...
        public WinEventEngine(Action<WinEventData, List<WinEventCategory>> consumer)
        {
            _consumer = consumer;
        ...
        public void StartThread()
        {
            ...
            {
                IsBackground = true,
                Name = "WinEventUtils.WinEventHook"
            };
            _hookThread.Start();
        ...
        public WinEventData[] SnapshotRing(int count)
        {
            var all = _ring.ToArray();
            if (all.Length <= count)
                return all;
            var tail = new WinEventData[count];
        ...
        private void SyncHook()
        {
            if (_hookRequested && _hook == IntPtr.Zero)
            {
                ...
                if (_hook == IntPtr.Zero)
                    Debug.WriteLine("WinEventUtils: SetWinEventHook failed, Win32 error " + Marshal.GetLastWin32Error());
            }
        ...
        private void OnWinEvent(IntPtr hHook, uint eventType, IntPtr hwnd, int idObject, int idChild, uint idThread, uint dwmsEventTime)
        {
            ...
            if (data == null)
                return;
            AddToRing(data);
            var cats = WinEventCategoryMap.CategoriesForEvent(eventType, data.ClassName);
            if (cats.Count == 0)
                return;
            try
            {
        ...
        /// </summary>
        private WinEventData Enrich(uint eventType, IntPtr hwnd)
        {
            ...
            if (sw.ElapsedMilliseconds > 30)
                Debug.WriteLine("WinEventUtils: enrichment took " + sw.ElapsedMilliseconds + " ms for " + data.Category);
            return data;
        }
