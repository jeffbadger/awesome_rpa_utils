...
```csharp
// Don't:
events.Subscribe(EventCategory.Windows, "{\"process\":\"notepad\",\"class\":\"#32770\",\"titleContains\":\"Save\"}", "sub", out string message);
...
events.Subscribe(EventCategory.Windows, filterJson, "sub", out message);
```
...
`EventFilterField` instead of naming a parameter — most useful when the
