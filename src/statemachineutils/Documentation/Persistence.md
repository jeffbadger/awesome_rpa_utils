...
| Saved file has a **duplicated field** (top level, a history entry, or a context key: two `started` fields, say) | `False` with a message naming it, same remedy. A JSON reader would silently keep the last one, so which value is right cannot be told and the file is treated as corrupt. |
