...
- **`PasteText`** requires the calling thread to be STA, as with any Windows clipboard
  access, and temporarily overwrites the system clipboard (restored in a `finally` block,
  but a crash between the set and restore could leave the pasted text on the clipboard).
