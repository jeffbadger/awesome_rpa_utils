...
All methods here except `CaptureAllScreensToClipboard` return `bool` with an
`out string message` and never throw — for these, replace Phase 2's
"exception condition on the invalid-input case" with an outcome condition
asserting `false` plus a non-null `message` on the invalid-input case (bad
...
- `CaptureAllScreensToFile`, `CaptureRegionToFile`, `CaptureWindowToFile`,
...
- `CaptureAllScreensToClipboard` (assert `Clipboard.ContainsImage()` — note: STA thread
  requirement)
...
- `GetCurrentSessionUptime` (against the actual interactive logon, like the group above; confirm the reported value is close to how long you've actually been logged on, and that it keeps increasing across repeated calls a few seconds apart), `GetSystemUptime` (no live-session dependency at all — `GetTickCount64` works in any process context — but still exercised manually here rather than in `SessionUtils.Tests`, consistent with every other method in this component; confirm against the machine's actual `net stats srv`/`systeminfo` uptime and that repeated calls increase; also worth a manual check across a sleep/hibernate cycle — whether `GetTickCount64` keeps advancing through suspend on the target hardware isn't verified here)
