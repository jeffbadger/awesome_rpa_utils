...
| [ReconciliationUtils](#reconciliationutils) | `ReconciliationAutomation` | 29 | 0 | 0 | Reconciles two JSON datasets or DataTables by business key and reports matches, differences, records missing on one side, duplicate keys and unusable rows, with exact decimal, text, Boolean, currency-gated money and explicit-format date and instant comparison and scalar results for routing. |
...
| `ListDialogControls` | `List<DialogControlInfo> ListDialogControls(IntPtr hDialog)` | Lists every control on a dialog — including nested controls — with its ID, text, and window class. Password edit boxes are listed without their text (`IsPassword` is true). |
...
Reconciles two JSON datasets or DataTables by business key and reports matches, differences, records missing on one side, duplicate keys and unusable rows, with exact decimal, text, Boolean, currency-gated money and explicit-format date and instant comparison and scalar results for routing.
