...
## `MoveMouseBezier(int x, int y, int durationMs = 500)`
...
```csharp
mouse.MoveMouseBezier(x: 850, y: 420); // default 500 ms, natural curve
mouse.LeftClick();
```
...
```csharp
mouse.MoveMouseBezier(x: ctaButtonX, y: ctaButtonY, durationMs: 1200);
mouse.LeftClick();
```
...
## `BezierClickAt(int x, int y, MouseButton button, int durationMs = 500)`
...
```csharp
mouse.BezierClickAt(x: 850, y: 420, MouseButton.Left);
```

## `BezierDoubleClickAt(int x, int y, MouseButton button, int durationMs = 500)`
...
```csharp
mouse.BezierDoubleClickAt(x: 340, y: 512, MouseButton.Left, durationMs: 800);
```

## `BezierDragAndDrop(int startX, int startY, int endX, int endY, int durationMs = 500)`
...
```csharp
mouse.BezierDragAndDrop(startX: 220, startY: 300, endX: 620, endY: 300, durationMs: 700);
```
